...
for python in python2 python3; do
    ...
    for i in $(ls -d tests/*/); do
        echo Testing $i
        diff $i/output.txt  <( cd $i && ./go.sh "$python $CHECKPROG" | sed ${FILT} | sed ${FILT2})
        diff $i/verbose-output.txt <( cd $i && ./go.sh "$python $CHECKPROG" --verbose | sed ${FILT} | sed ${FILT2})
        diff $i/output.html <(cd $i && ./go.sh "$python $CHECKPROG" --html /tmp/html.tmp > /dev/null && sed ${FILT} /tmp/html.tmp | sed ${FILT2} && rm /tmp/html.tmp)
        diff $i/output.json <(cd $i && ./go.sh "$python $CHECKPROG" --json /tmp/json.tmp > /dev/null && sed ${FILT} /tmp/json.tmp | sed ${FILT2} && rm /tmp/json.tmp)
        rm -f /tmp/try-harder.html /tmp/try-harder.json /tmp/try-harder.txt
        ( cd $i && ./go.sh "$python $CHECKPROG" --tryharder --html /tmp/try-harder.html --json /tmp/try-harder.json || true) > /tmp/try-harder.txt
        ...
        ( cd $i && ./go.sh "$python $CHECKPROG" --verbose --html /tmp/verbose-test.html --json /tmp/verbose-test.json || true) > /tmp/verbose-test.txt
